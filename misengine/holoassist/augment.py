# Usage: python augment.py --split train [--input train_base.xlsx] [--output train.xlsx]
#   Defaults: --input {split}_base.xlsx, --output {split}.xlsx
import pandas as pd
import random 
import argparse
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Build misalignment-augmented split for HoloAssist.")
    parser.add_argument('--split', type=str, required=True, choices=['train', 'validation', 'test'], help='Which split to augment')
    parser.add_argument('--input', type=str, default=None, help='Input xlsx (default: {split}_base.xlsx)')
    parser.add_argument('--output', type=str, default=None, help='Output xlsx (default: {split}.xlsx)')
    args = parser.parse_args()

    if args.input is None:
        args.input = f"{args.split}_base.xlsx"
    if args.output is None:
        args.output = f"{args.split}.xlsx"

    df = pd.read_excel(args.input)

    # Filter out rows that have "None" or empty values
    df = df[
        (df["V"] != "None") & (df["V"] != "") &
    ...
    # Organize rows by combination of V and ARG1. 'x' refers to the rows that meet that combination of 'V' and 'ARG1' values
