...
class EKDataset(Dataset):
    ...
    def video_loader(self, video_id, sf, ef):
        frames_dir = os.path.join(self.args.root1, video_id.split("_")[0], "rgb_frames", video_id)
        ...
        for frame_num in frames:
            if not os.path.exists(os.path.join(frames_dir, "frame_" + str(frame_num).zfill(10) + ".jpg")):
                fp = os.path.join(frames_dir, "frame_" + str(frame_num).zfill(10) + ".jpg")
                raise FileNotFoundError(f"The file '{fp}' does not exist.")

            frame_tensor = torchvision.io.read_image(os.path.join(frames_dir, "frame_" + str(frame_num).zfill(10) + ".jpg"), mode= ImageReadMode.UNCHANGED)
    ...
    def __getitem__(self, index):
        ...
        frames = self.video_loader(video_id, int(sf), int(ef))
