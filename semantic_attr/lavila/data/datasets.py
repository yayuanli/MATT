...
def video_loader(root, vid, second, end_second=None, chunk_len=300, fps=30, clip_length=32, jitter=False):
    ...
    print(f"Shape of frame_ids: {frame_ids.size()}")
    ...
    print(f"Shape of numpy frames: {frames.shape}")

    frames = [torch.tensor(frame, dtype=torch.float32) for frame in frames]

    print(f"Shape of frames after tensor coversion: {frames.shape}")
    print(f"Shape of frames after stacking: {torch.stack(frames, dim=0).shape}")
...
def video_loader_by_frames(root, vid, frame_ids):
    ...
    try:
        frames = vr.get_batch(frame_ids).asnumpy()
        print(f"Length of frame_ids: {len(frame_ids)}")
        print(f"Shape of numpy frames: {frames.shape}")
        frames = [torch.tensor(frame, dtype=torch.float32) for frame in frames]
        #print(f"Shape of frames after tensor coversion: {frames.size}")
...
class VideoClassyDataset(VideoCaptionDatasetBase):
    ...
    def __getitem__(self, i):
        ...
        if self.transform is not None:
            frames = self.transform(frames)
            print(f"Shape of frames after transformation: {frames.shape}")
