...
class EgoPERDataset(Dataset):
    ...
    def video_loader(self, video_id, sf, ef):
        if video_id + "_frames" in self.root1:
            frames_dir = os.path.join(self.args.root1, video_id + "_frames")
        else:
        ...
        indices = np.linspace(sf, ef - 1, self.args.clip_length, endpoint=True).astype(int)
        ...
        for frame_num in frames:
            if not os.path.exists(os.path.join(frames_dir, str(frame_num).zfill(6) + ".png")):
                raise FileNotFoundError(f"The file '{os.path.join(frames_dir, str(frame_num).zfill(6) + '.png')}' does not exist.")
            
            frame_tensor = torchvision.io.read_image(os.path.join(frames_dir, str(frame_num).zfill(6) + ".png"), mode= ImageReadMode.UNCHANGED)
    ...
    def __getitem__(self, index):
        ...
        video_id = row["video_id"] # effectively the clip1 uid
        sf = row["start_frame"] # effectively the clip1_start_frame
        ef = row["end_frame"] # effectively the clip1_end_frame

        frames = self.video_loader(video_id, sf, ef)
