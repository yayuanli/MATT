...
class VideoPatchEmbed(nn.Module):
    ...
    def forward(self, x):
        ...
        x = x.view(-1, C, H, W)  #Merge the B and F dimensions
        x = self.proj(x) #Shape is (batch_size * num_frames, 1024, 16, 16) afterwards
        return x
...
class VarAttention(nn.Module):
    ...
    def forward(self, x, einops_from, einops_to, einops_dims):
        ...
        q, k, v = map(lambda t: rearrange(t, 'b n (h d) -> (b h) n d', h=h).clone(), (q, k, v))
...
class SpaceTimeTransformer(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=1000, embed_dim=768, depth=12,
                 ...
                 num_frames=32, time_init='rand', attention_style='frozen-in-time', ln_pre=False,
    ...
    def forward_features(self, x, use_checkpoint=False, cls_at_last=True):
        ...
        x = self.patch_embed(x) #Shape is torch.Size([20, 1024, 16, 16]) afterwards
       #print(f"Shape of x after line 348: {x.shape}") //Height and width are 16

        x = x.flatten(2).transpose(2, 1) #Shape is (20, 256, 1024)
        #print(f"Shape of x after line 349: {x.shape}")

        x = x.reshape(b, -1, self.patch_embed.embed_dim) #This essentially 

        #print(f"Shape of x after line 350: {x.shape}") #shape is torch.Size([1, 5120, 1024]). The first dimension is the number of batches, the second is the processed pixels in an entire video, and the third is the output of the path_embed model
        #5120 is number of frames times patches of frames. 
        
        BF = x.shape[0]
        cls_tokens = self.cls_token.expand(BF, -1, -1)  # stole cls_tokens impl from Phil Wang, thanks
        x = torch.cat((cls_tokens, x), dim=1)

        #print(f"Shape of x after line 356: {x.shape}") #Shape is torch.Size([1, 5121, 1024]). The CLS token (global representation) was added at the first index of the middle dimension
       ...
       # print(f"Shape of x after line 369: {x.shape}") #torch.Size([1, 5121, 1024])

        if self.ln_pre is not None:
            x = self.ln_pre(x)
        x = self.pos_drop(x)
        n = self.patches_per_frame
       # print(f"Num patches per frame: {n}")
        f = curr_frames

        #print(f"Shape of x after line 377: {x.shape}")
       ...
       # print(f"Shape of x after line 382: {x.shape}") #torch.Size([1, 5121, 1024])

        if cls_at_last:
            #x = self.norm(x)[:, 0]
            x = self.norm(x)
            #print(f"Shape of x after line 390: {x.shape}") #torch.Size([1, 5121, 1024])
            #x = self.pre_logits(x)
    ...
    def forward(self, x, use_checkpoint=False):
        ...
        #print(f"Shape of x after line 388: {x.shape}") #torch.Size([1, 20, 3, 224, 224]). 1 is the batch size, 20 is the num of frames, 3 is num channels, and 224 is the height and width resized of the frames I assume
        x = self.forward_features(x, use_checkpoint=use_checkpoint)
        x = self.head(x)
        return x #torch.Size([1, 5121, 1024])
